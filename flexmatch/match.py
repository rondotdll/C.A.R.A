...
from flexmatch import strings
from flexmatch.levenshtein import min_distance
...
async def match_user(input: str, context: Message) -> User:
    ...
    uname_matches, uname_match_type, uname_distance = match(input, list(guild_username_map.keys()))
    ...
    if len(guild_nickname_map) > 0:
        nick_matches, nick_match_type, nick_distance = match(input, list(guild_nickname_map.keys()))
        ...
        if nick_match_type == MatchType.editdist:
            ...
            if uname_distance < nick_distance:
                ...
                raise NameConflictError(
                    list(set(flatten(uname_matches, guild_username_map) + flatten(nick_matches, guild_nickname_map)))
                )

        elif nick_match_type == MatchType.substr:
            # both are substring matches...
            raise NameConflictError(
                flatten(uname_matches, guild_username_map) + flatten(nick_matches, guild_nickname_map))
        else:
            raise NoNameError
...
def match(input: str, vals: list[str]) -> (list[str], MatchType, int):
    ...
    def sanitize(value: str) -> str:
        always_acceptable = 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789'
        output = [c if c in always_acceptable
                  or c in value else '' for c in value]  # if there is a non-normalized character in our input, keep it
        return ''.join(output)
    ...
    if len(substring_match) != 0 and lvshtn_distance != 0:
        ...
        if lvshtn_distance < (len(input) / 2):
            ...
            if len(viable_substr_matches) > 0:
                return [sanitized_map[viable_substr] for viable_substr in viable_substr_matches], MatchType.substr, -1
        else:
            return [sanitized_map[match] for match in substring_match], MatchType.substr, -1

    # if "input" is less than 50% similar to any "vals"
    elif lvshtn_distance >= len(input) * 1 / 2:
        return None, MatchType.none, -1

    return [sanitized_map[match] for match in lvshtn_match], MatchType.editdist, lvshtn_distance